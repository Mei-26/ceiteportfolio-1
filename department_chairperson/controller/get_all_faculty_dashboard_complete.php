...
function uicard($card_number,$image,$name,$position,$specialization,$contacts,$icon)
{
    ...
    <tr>
        <td rowspan="5" width="80px">
            <img data-card="'.$card_number.'" class="toggle-btn cursor-pointer rounded-lg" style="height:60px!important; width:100%!important;" src="../uploads/faculty/' . $image . '" alt="">
        </td>
    </tr>
    <tr>
        <td width="120px" data-card="'.$card_number.'" class="toggle-btn px-4 cursor-pointer " colspan="3" style="font-size:12px;">' . $name . '</td>
        <td rowspan="4" data-card="'.$card_number.'" class="toggle-btn px-4 cursor-pointer  text-start " style="font-size:12px;">
            ' . $icon . '
        </td>
    </tr>
    <tr>
        <td colspan="3" data-card="'.$card_number.'" class="toggle-btn px-4 cursor-pointer " style="font-size:12px;">' . $position . '</td>
    </tr>
    <tr>
        <td colspan="3" data-card="'.$card_number.'" class="toggle-btn px-4 cursor-pointer " style="font-size:12px;">' . $specialization . '</td>
    </tr>
    <tr>
        <td colspan="3" data-card="'.$card_number.'" class="toggle-btn px-4 cursor-pointer " style="font-size:12px;">' . $contacts . '</td>
    </tr>';
   echo '<tr>
    <td></td>
    <td></td>
...
function toglerCard($card_number,$toglerCard)
{
    ...
    echo '<!-- Collapsible Section -->
        <tr data-card_col="'.$card_number.'"  class="collapse hidden">
            <td colspan="5">
                <h1 style="font-size:12px" class="text-gray-400 font-bold mt-5 mb-2">
                    DOCUMENT STATUS - <span class="text-green-500">COMPLETE</span>
                </h1>
            </td>
        </tr>
        <tr data-card_col="'.$card_number.'" class="collapse hidden">
    ...
    foreach ($array as $key => $value) {
        ...
        <tr style="position: relative; z-index: 10;" data-card_col="'.$card_number.'" class="collapse hidden">
            <td colspan="5">
                <div class="card shadow-lg rounded-lg border text-center border-gray-200 p-2 mb-2">
                    <div class="grid grid-cols-3 items-center">
                        <!-- Link Section -->
